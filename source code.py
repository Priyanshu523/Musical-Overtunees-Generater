...
class NotePlayer:
    ...
    def __init__(self):
        ...
        self.notes = {}
    # add a note
    def add(self, wavefile):
        self.notes[wavefile] = pygame.mixer.Sound(wavefile)
    # play a note
    def play(self, wavefile):
        try:
            self.notes[wavefile].play()
        except:
...
def main():
    ...
    for name, freq in list(pmNotes.items()):
        wavefile = name + '.wav' 
        if not os.path.exists(wavefile) or args.display:
            data = generateNote(freq) 
            print('creating ' + wavefile + '...')
            writeWAVE(wavefile, data) 
        else:
